...
pub fn verify_block_family<BC>(header: &Header, bytes: &[u8], engine: &Engine, bc: &BC) -> Result<(), Error>
	where BC: BlockProvider,
{
	...
	if num_uncles != 0 {
		...
		for uncle in Rlp::new(bytes).at(2).iter().map(|rlp| rlp.as_val::<Header>()) {
			...
			let uncle_parent = try!(bc.block_header(&uncle.parent_hash)
			                          .ok_or_else(|| Error::from(BlockError::UnknownUncleParent(uncle.parent_hash.clone()))));
			for _ in 0..depth {

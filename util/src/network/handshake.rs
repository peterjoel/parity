...
impl Handshake {
	...
	pub fn new(token: StreamToken,
	           id: Option<&NodeId>,
	           socket: TcpStream,
	           nonce: &H256,
	           stats: Arc<NetworkStats>)
	           -> Result<Handshake, UtilError> {
		Ok(Handshake {
	...
	pub fn register_socket<Host: Handler<Timeout = Token>>(&self,
	                                                       reg: Token,
	...
	pub fn update_socket<Host: Handler<Timeout = Token>>(&self,
	                                                     reg: Token,
	                                                     event_loop: &mut EventLoop<Host>)
	                                                     -> Result<(), UtilError> {

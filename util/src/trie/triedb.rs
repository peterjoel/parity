...
impl<'db> TrieDB<'db> {
	...
	fn get_raw_or_lookup<'a>(&'a self, node: &'a [u8]) -> &'a [u8] {
		...
		match r.is_data() && r.size() == 32 {
			true => {
				self.db
				    .lookup(&r.as_val::<H256>())
				    .unwrap_or_else(|| panic!("Not found! {:?}", r.as_val::<H256>()))
			}
			false => node,
		}
	}
...
fn iterator() {
	...
	assert_eq!(d.iter().map(|i| i.to_vec()).collect::<Vec<_>>(),
	           TrieDB::new(&memdb, &root).iter().map(|x| x.0).collect::<Vec<_>>());
	assert_eq!(d,
	           TrieDB::new(&memdb, &root).iter().map(|x| x.1).collect::<Vec<_>>());
}

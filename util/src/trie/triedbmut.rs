...
impl<'db> TrieDBMut<'db> {
		...
		where 'a: 'key
	{
		match Node::decoded(node) {
			...
			Node::Branch(ref nodes, value) => {
				match key.is_empty() {
					true => value,
					false => {
						self.get_from_node(self.get_raw_or_lookup(nodes[key.at(0) as usize]),
						                   &key.mid(1))
					}
				}
			}
	...
	fn insert_ns(&mut self, key: &NibbleSlice, value: &[u8]) {
		...
		let root_rlp = self.augmented(self.db.lookup(&self.root).expect("Trie root not found!"),
		                              key,
		                              value,
		                              &mut todo);
		self.apply(todo);
	...
	fn remove_ns(&mut self, key: &NibbleSlice) {
		...
		// determine what the new root is, insert new nodes and remove old as necessary.
		let mut todo: Journal = Journal::new();
		match self.cleared_from_slice(self.db.lookup(&self.root).expect("Trie root not found!"),
		                              key,
		                              &mut todo) {
			Some(root_rlp) => {
				self.apply(todo);
	...
	fn take_node<'a, 'rlp_view>(&'a self,
	                            rlp: &'rlp_view Rlp<'a>,
	                            journal: &mut Journal)
	                            -> &'a [u8]
		where 'a: 'rlp_view
	{
		if rlp.is_list() {
			...
			let r = self.db.lookup(&h).unwrap_or_else(|| {
				println!("Node not found! rlp={:?}, node_hash={:?}",
				         rlp.as_raw().pretty(),
				         h);
				println!("Journal: {:?}", journal);
				panic!();
			});
			trace!("take_node {:?} (indirect for {:?})",
			       rlp.as_raw().pretty(),
			       r);
	...
	fn augmented(&self,
		...
		match old_rlp.prototype() {
			Prototype::List(17) => {
				...
				let mut s = RlpStream::new_list(17);
				let index = if partial.is_empty() {
					16
				} else {
					partial.at(0) as usize
				};
				for i in 0..17 {
					match index == i {
						// not us - leave alone.
						...
						true if old_rlp.at(i).is_empty() => {
							journal.new_node(Self::compose_leaf(&partial.mid(1), value), &mut s)
						}
						// original has something there already; augment.
						true => {
							let new = self.augmented(self.take_node(&old_rlp.at(i), journal),
							                         &partial.mid(1),
							                         value,
							                         journal);
			...
			Prototype::List(2) => {
				...
				match (is_leaf, partial.common_prefix(&existing_key)) {
					(true, cp) if cp == existing_key.len() &&
					              partial.len() == existing_key.len() => {
					...
					(_, 0) => {
						...
						for i in 0..17 {
							match is_leaf {
								...
								true => {
									journal.new_node(Self::compose_leaf(&existing_key.mid(1),
									                                    old_rlp.at(1).data()),
									                 &mut s)
								}
								// additional work required after branching.
								false => {
									journal.new_node(Self::compose_extension(&existing_key.mid(1),
									                                         old_rlp.at(1)
									                                                .as_raw()),
									                 &mut s)
								}
					...
					(_, cp) if cp == existing_key.len() => {
						...
						let downstream_node: Bytes = match is_leaf {
							...
							true => {
								self.augmented(&Self::compose_stub_branch(old_rlp.at(1).data()),
								               &partial.mid(cp),
								               value,
								               journal)
							}
							false => {
								self.augmented(self.take_node(&old_rlp.at(1), journal),
								               &partial.mid(cp),
								               value,
								               journal)
							}
						...
						trace!("create_extension partial: {:?}, downstream_node: {:?}",
						       existing_key,
						       downstream_node.pretty());
						let mut s = RlpStream::new_list(2);
					...
					(_, cp) => {
						...
						trace!("partially-shared-prefix (exist={:?}; new={:?}; cp={:?}): \
						        AUGMENT-AT-END",
						       existing_key.len(),
						       partial.len(),
						       cp);
						...
						let low = Self::compose_raw(&existing_key.mid(cp),
						                            old_rlp.at(1).as_raw(),
						                            is_leaf);
	...
	fn fixed_indirection<'a>(n: Node<'a>, journal: &mut Journal) -> MaybeChanged<'a> {
		match n {
			Node::Extension(partial, payload) if payload.len() >= 32 &&
			                                     Rlp::new(payload).is_list() => {
				// make indirect
				MaybeChanged::Changed(Node::Extension(partial,
				                                      &Node::decoded(payload)
					                                       .encoded_and_added(journal))
					                      .encoded())
			}
		...
		where 'a: 'b
	{
		...
		match n {
			Node::Branch(nodes, node_value) => {
				...
				trace!("branch: used_index={:?}, node_value={:?}",
				       used_index,
				       node_value);
	...
	fn cleared(&self, n: Node, partial: &NibbleSlice, journal: &mut Journal) -> Option<Bytes> {
		...
		match (n, partial.is_empty()) {
			...
			(Node::Branch(payloads, _), true) => {
				Some(Self::encoded(self.fixed(Node::Branch(payloads, None), journal)))
			}	// matched as leaf-branch - give back fixed branch with it.
			(Node::Branch(payloads, value), false) => {
				// Branch with partial left - route, clear, fix.
				let i: usize = partial.at(0) as usize;
				trace!("branch-with-partial node[{:?}]={:?}",
				       i,
				       payloads[i].pretty());
				self.cleared(self.get_node(payloads[i]), &partial.mid(1), journal)
				    .map(|new_payload| {
					    trace!("branch-new-payload={:?}; delete-old={:?}",
					           new_payload.pretty(),
					           payloads[i].pretty());
			...
			(Node::Extension(node_partial, node_payload), _) => {
				trace!("extension partial={:?}, payload={:?}",
				       node_partial,
				       node_payload.pretty());
				match node_partial.common_prefix(partial) {
					cp if cp == node_partial.len() => {
						...
						self.cleared(self.get_node(node_payload),
						             &partial.mid(node_partial.len()),
						             journal)
						    .map(|new_payload| {
							    trace!("extension-new-payload={:?}; delete-old={:?}",
							           new_payload.pretty(),
							           node_payload.pretty());
							    // downsteam node needed to be changed.
							    journal.delete_node(node_payload);
							    // return fixed up new node.
							    Self::encoded(self.fixed(Node::Extension(node_partial,
							                                             &new_payload),
							                             journal))
							   })
					}
...
mod tests {
	...
	fn remove_to_empty() {
		...
		println!("********************** keys remaining {:?}",
		         t1.db_items_remaining());
	...
	fn insert_big_value() {
		...
		assert_eq!(*t.root(),
		           trie_root(vec![(vec![0x01u8, 0x23], big_value0.to_vec()),
	...
	fn insert_duplicate_value() {
		...
		assert_eq!(*t.root(),
		           trie_root(vec![(vec![0x01u8, 0x23], big_value.to_vec()),

    ...
    where 'a: 'view
{
	...
	pub fn new(data: &[u8]) -> NibbleSlice {
		NibbleSlice::new_offset(data, 0)
	}

	/// Create a new nibble slice with the given byte-slice with a nibble offset.
	pub fn new_offset(data: &'a [u8], offset: usize) -> NibbleSlice {
		NibbleSlice {
			data: data,
			offset: offset,
	...
	pub fn new_composed(a: &'a NibbleSlice, b: &'a NibbleSlice) -> NibbleSlice<'a> {
		NibbleSlice {
			data: a.data,
			offset: a.offset,
	...
	pub fn len(&self) -> usize {
		(self.data.len() + self.data_encode_suffix.len()) * 2 - self.offset -
...
mod tests {
	...
	fn from_encoded() {
		...
		assert_eq!((n.mid(1), false),
		           NibbleSlice::from_encoded(&[0x11, 0x23, 0x45]));
		assert_eq!((n.mid(1), true),

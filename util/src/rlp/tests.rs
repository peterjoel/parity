...
fn encode_u256() {
	let tests = vec![ETestPair(U256::from(0u64), vec![0x80u8]),
	                 ...
	                 ETestPair(U256::from_str("8090a0b0c0d0e0f009102030405060770000000000000001\
	                                           00000000000012f0")
		                           .unwrap(),
	                           vec![0xa0, 0x80, 0x90, 0xa0, 0xb0, 0xc0, 0xd0, 0xe0, 0xf0, 0x09,
	                                0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x77, 0x00, 0x00, 0x00,
...
fn encode_str() {
	let tests = vec![ETestPair("cat", vec![0x83, b'c', b'a', b't']),
	                 ...
	                 ETestPair("Lorem ipsum dolor sit amet, consectetur adipisicing elit",
	                           vec![0xb8, 0x38, b'L', b'o', b'r', b'e', b'm', b' ', b'i', b'p',
	                                b's', b'u', b'm', b' ', b'd', b'o', b'l', b'o', b'r', b' ',
	                                b's', b'i', b't', b' ', b'a', b'm', b'e', b't', b',', b' ',
	                                b'c', b'o', b'n', b's', b'e', b'c', b't', b'e', b't', b'u',
	                                b'r', b' ', b'a', b'd', b'i', b'p', b'i', b's', b'i', b'c',
	                                b'i', b'n', b'g', b' ', b'e', b'l', b'i', b't'])];
	run_encode_tests(tests);
}
...
fn encode_address() {
	...
	let tests = vec![ETestPair(Address::from_str("ef2d6d194084c2de36e0dabfce45d046b37d1106")
		                           .unwrap(),
	                           vec![0x94, 0xef, 0x2d, 0x6d, 0x19, 0x40, 0x84, 0xc2, 0xde, 0x36,
	                                0xe0, 0xda, 0xbf, 0xce, 0x45, 0xd0, 0x46, 0xb3, 0x7d, 0x11,
	                                0x06])];
	run_encode_tests(tests);
}
...
fn run_decode_tests<T>(tests: Vec<DTestPair<T>>)
	where T: rlp::RlpDecodable + fmt::Debug + cmp::Eq
{
	for t in &tests {
...
fn decode_untrusted_u256() {
	let tests = vec![DTestPair(U256::from(0u64), vec![0x80u8]),
	                 ...
	                 DTestPair(U256::from(0xffffffffu64),
	                           vec![0x84, 0xff, 0xff, 0xff, 0xff]),
	                 DTestPair(U256::from_str("8090a0b0c0d0e0f009102030405060770000000000000001\
	                                           00000000000012f0")
		                           .unwrap(),
	                           vec![0xa0, 0x80, 0x90, 0xa0, 0xb0, 0xc0, 0xd0, 0xe0, 0xf0, 0x09,
	                                0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x77, 0x00, 0x00, 0x00,
...
fn decode_untrusted_address() {
	...
	let tests = vec![DTestPair(Address::from_str("ef2d6d194084c2de36e0dabfce45d046b37d1106")
		                           .unwrap(),
	                           vec![0x94, 0xef, 0x2d, 0x6d, 0x19, 0x40, 0x84, 0xc2, 0xde, 0x36,
	                                0xe0, 0xda, 0xbf, 0xce, 0x45, 0xd0, 0x46, 0xb3, 0x7d, 0x11,
	                                0x06])];
...
fn test_rlp_json() {
	...
	execute_tests_from_directory::<rlptest::RlpStreamTest, _>("json-tests/json/rlp/stream/*.json",

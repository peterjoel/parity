    ...
    where 'a: 'view
{
	...
	fn at(&'view self, index: usize) -> Self::Item {
		...
		let (mut bytes, to_skip) = match c.index <= index {
			true => {
				(try!(UntrustedRlp::consume(self.bytes, c.offset)),
				 index - c.index)
			}
...
fn test_rlp_display() {
	...
	let data = "f84d0589010efbef67941f79b2a056e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622f\
	            b5e363b421a0c5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470"
		           .from_hex()
		           .unwrap();
	let rlp = UntrustedRlp::new(&data);
	assert_eq!(format!("{}", rlp),
	           "[\"0x05\", \"0x010efbef67941f79b2\", \

		...
		match *json {
			Json::String(ref s) => {
				match s.len() % 2 {
					...
					_ => {
						FromHex::from_hex(&("0".to_owned() + &(clean(s).to_owned()))[..])
...
impl FromJson for BTreeMap<H256, H256> {
	fn from_json(json: &Json) -> Self {
		match *json {
			Json::Object(ref o) => {
				o.iter()
				 .map(|(key, value)| (x!(&u256_from_str(key)), x!(&U256::from_json(value))))
				 .collect()
			}
			_ => BTreeMap::new(),
		}
...
fn h256_from_json() {
	...
	assert_eq!(H256::from_str("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
		           .unwrap(),
	           v);
...
#[test]
fn simple_types() {
	let j = Json::from_str("{ \"null\": null, \"empty\": \"\", \"int\": 42, \"dec\": \"42\", \
	                        \"hex\": \"0x2a\" }")
		        .unwrap();
